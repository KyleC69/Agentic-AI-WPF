...
public class LearningHtmlRunner
{
    ...
    public LearningHtmlRunner(IHeadlessBrowser headlessBrowser, IDocRepository docRepository, ILogger<LearningHtmlRunner> logger, IOptions<IngestionSettings> settings, IOllamaApiClient ollamaApiClient)
    {
    ...
    private List<(HtmlNode Heading, HtmlNode Paragraph)> CombineSuccessiveParagraphs(HtmlNodeCollection content)
    {
        ...
        {
            return new List<(HtmlNode Heading, HtmlNode Paragraph)>();
        }

        var results = new List<(HtmlNode Heading, HtmlNode Paragraph)>();
        HtmlNode currentHeading = null;
        List<string> paragraphFragments = new();
        ...
        bool IsHeading(HtmlNode node) => node.Name.StartsWith("h", StringComparison.OrdinalIgnoreCase) && node.Name.Length == 2 && char.IsDigit(node.Name[1]);
    ...
    public async Task EmbedRemoteKnowledgeSource(CancellationToken cancellationToken = default)
    {
        ...
        using (SqlCommand cmd = new(select, conn))
        {
            ...
            {
                while (reader.Read()) existingUrls.Add(reader["og_url"].ToString());
            }
        }
        ...
        foreach (var pg in crawlList)
        {
            ...
            var embedding = new SqlVector<float>(vectory.Embeddings[0]); // 
            RemoteRAG rag = new()
            {
        ...
        foreach (var pg in crawlList)
        {
            ...
            try
            {
                ...
                static int? TryGetHeadingLevel(string name)
                {
                    ...
                    if (name.Length == 2 && name.StartsWith("h", StringComparison.OrdinalIgnoreCase) && char.IsDigit(name[1]))
                    {
                ...
                foreach ((HtmlNode Heading, HtmlNode Paragraph) pair in combined)
                {
                    var headingText = pair.Heading?.InnerText?.Trim();
                    var paragraphText = pair.Paragraph?.InnerText?.Trim();
                    ...
                    var level = TryGetHeadingLevel(pair.Heading?.Name);
                    var semanticUid = HashUtils.ComputeSemanticUidForSection(address, sectionContent, level ?? 2, pair.Heading?.InnerStartIndex ?? 0);
