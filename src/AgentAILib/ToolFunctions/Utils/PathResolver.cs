...
internal static class PathResolver
{
    ...
    {
        var normalizedRootsResult = NormalizeAllowedRoots(allowedRoots);
        if (!normalizedRootsResult.Success)
        {
            return normalizedRootsResult;
        }

        IReadOnlyList<string> existingRoots = normalizedRootsResult.Value!.Where(Directory.Exists).ToList().AsReadOnly();
    ...
    private static string GetNearestExistingPath(string path)
    {
        ...
        while (!string.IsNullOrEmpty(currentPath) && !File.Exists(currentPath) && !Directory.Exists(currentPath)) currentPath = Path.GetDirectoryName(currentPath);
    ...
    private static bool IsPathWithinRoot(string candidatePath, string normalizedRoot)
    {
        ...
        return !relativePath.Equals("..", StringComparison.Ordinal) && !relativePath.StartsWith($"..{Path.DirectorySeparatorChar}", StringComparison.Ordinal) && !relativePath.StartsWith($"..{Path.AltDirectorySeparatorChar}", StringComparison.Ordinal) && !Path.IsPathRooted(relativePath);
    }
    ...
    {
        var candidatePathResult = TryNormalizeFullPath(path);
        ...
        return ToolResult<ResolvedPath>.Ok(new ResolvedPath { RequestedPath = sanitizedPath, FullPath = candidatePath, AllowedRoot = matchedRoot });
    }
    ...
    private static ToolResult<ResolvedPath> ResolveExistingPath(string path, IEnumerable<string>? allowedRoots, ResolvedPathKind expectedKind)
    {
        ...
        var normalizedRoots = normalizedRootsResult.Value!;
        ...
        {
            var candidatePathResult = TryNormalizeFullPath(Path.Combine(normalizedRoot, sanitizedPath));
            if (!candidatePathResult.Success)
            {
                continue;
            }
            ...
            if (IsExpectedKind(candidatePath, expectedKind))
            {
                return ToolResult<ResolvedPath>.Ok(new ResolvedPath { RequestedPath = sanitizedPath, FullPath = candidatePath, AllowedRoot = normalizedRoot });
            }
    ...
    internal static ToolResult<ResolvedPath> ResolveFilePathForWrite(string path, IEnumerable<string>? allowedRoots)
    {
        ...
        var normalizedRoots = normalizedRootsResult.Value!;
        var candidatePathResult = Path.IsPathRooted(sanitizedPath) ? TryNormalizeFullPath(sanitizedPath) : TryNormalizeFullPath(Path.Combine(normalizedRoots[0], sanitizedPath));
        if (!candidatePathResult.Success)
        {
            return ToolResult<ResolvedPath>.Fail(candidatePathResult.Error!);
        }
    ...
    private static bool TryNormalizeRoot(string? root, out string? normalizedRoot)
    {
        ...
        var fullPathResult = TryNormalizeFullPath(Environment.ExpandEnvironmentVariables(root.Trim()));
