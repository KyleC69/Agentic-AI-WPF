...
/// </summary>
[Description("Runs read-only PowerShell diagnostics on the local Windows system. " + "Allowed verbs: Get, Select, Where, ForEach, Format, Measure, Test, Find, Search, Compare, Sort, Group, " + "ConvertTo, ConvertFrom, Resolve, Show, Split, Join, Out, Write. " + "Commands using any other verb, the call operator '&', dot-sourcing, or destructive aliases are blocked.")]
public sealed class PowerShellTool
{
    ...
    private static async Task<ToolResult<string>> ExecuteCommandAsync(string command, int timeoutMs)
    {
        ...
        if (psExe is null)
            return ToolResult<string>.Fail("No compatible PowerShell executable (pwsh.exe or powershell.exe) was found on PATH.");
        ...
        if (!completed)
        {
            ...
            return ToolResult<string>.Fail($"PowerShell command timed out after {timeoutMs / 1_000} seconds.");
        }
        ...
        if (process.ExitCode != 0 && string.IsNullOrWhiteSpace(output))
        {
            ...
            return ToolResult<string>.Fail($"PowerShell exited with code {process.ExitCode}. Error: {snippet}");
        }
    ...
    [Description("Execute a read-only PowerShell command or pipeline on the local machine. " + "Returns captured stdout as text, or an error explanation if the command is blocked or fails. " + "Example: 'Get-Process | Sort-Object CPU -Descending | Select-Object -First 10 | Format-Table Name,CPU,WorkingSet'")]
    public async Task<ToolResult<string>> RunReadOnly([Description("The PowerShell command or pipeline to run. Must use only allowed read-only verbs.")] string command, [Description("Maximum seconds to wait before aborting. Minimum 1, maximum 30. Defaults to 20.")] int timeoutSeconds = 20)
    {
        ...
        {
            return ToolResult<string>.Fail($"Timeout must be between {MIN_TIMEOUT_SECONDS} and {MAX_TIMEOUT_SECONDS} seconds.");
        }
