...
public sealed class ForensicChatClient : DelegatingChatClient
{
    ...
    public override async Task<ChatResponse> GetResponseAsync(IEnumerable<ChatMessage> messages, ChatOptions? options = null, CancellationToken cancellationToken = default)
    {
        ...
        SafeLog(LogLevel.Information, "[Forensic:{Id}] ENTER GetResponseAsync(messages)", id);

        List<string> anomalies = new();
        DetectRequest(messages, "GetResponseAsync(messages)", anomalies);

        ChatResponse response = await base.GetResponseAsync(messages, options, cancellationToken);

        DetectResponse(response, "GetResponseAsync(messages)", anomalies);

        LogAnomalies(id, anomalies);
        SafeLog(LogLevel.Information, "[Forensic:{Id}] EXIT GetResponseAsync(messages)", id);
    ...
    {
        var id = CorrelationId();
        SafeLog(LogLevel.Information, "[Forensic:{Id}] ENTER GetStreamingResponseAsync(messages)", id);

        List<string> anomalies = new();
        DetectRequest(messages, "GetStreamingResponseAsync(messages)", anomalies);
        ...
        {
            sawAny = true;
            DetectStreaming(update, "GetStreamingResponseAsync(messages)", anomalies);
            yield return update;
        }
        ...
        LogAnomalies(id, anomalies);
        SafeLog(LogLevel.Information, "[Forensic:{Id}] EXIT GetStreamingResponseAsync(messages)", id);
    }
    ...
    private void LogAnomalies(string id, List<string> anomalies)
    {
        ...
        SafeLog(LogLevel.Warning, "[Forensic:{Id}] Anomalies detected:\n - {A}", id, string.Join("\n - ", anomalies));
    }
