...
using ChatMessageExtensions = AgentAILib.Models.Extensions.ChatMessageExtensions;
...
public sealed class TokenAccountingMiddleware : DelegatingChatClient
{
    ...
    public override async IAsyncEnumerable<ChatResponseUpdate> GetStreamingResponseAsync(IEnumerable<ChatMessage> messages, ChatOptions? options = null, [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        if (sawAny)
        {
            await OnStreamingCompletedAsync(requestContext, estimatedStreamingOutputTokens, lastUsage, cancellationToken).ConfigureAwait(false);
        }
    }
    ...
    internal Task OnStreamingCompletedAsync(MiddlewareRequestContext requestContext, int estimatedStreamingOutputTokens, UsageDetails? usage, CancellationToken cancellationToken)
    {
        ...
        var inputTokens = ClampToInt(usage?.InputTokenCount ?? requestContext.EstimatedInputTokens);
        var outputTokens = ClampToInt(usage?.OutputTokenCount ?? estimatedStreamingOutputTokens);
        var cachedInputTokens = ClampToInt(usage?.CachedInputTokenCount ?? 0);
