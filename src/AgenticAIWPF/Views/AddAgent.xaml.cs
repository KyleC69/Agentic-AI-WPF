...
/// </summary>
public partial class AddAgent : MetroWindow
{
    public AddAgent(BlankViewModel viewModel)
    {
        InitializeComponent();
    ...
    public BlankViewModel ParentViewModel { get; set; }

    internal PredefinedAgentType SelectedAgentType { get; set; }
    ...
    {
        AgentDescriptor agent = new();
        agent.Name = AgentName.Text;
        agent.Description = AgentDescription.Text;
        agent.Instructions = GetDefaultInstructions((PredefinedAgentType)AgentTypeComboBox.SelectedItem);
        agent.Model = AIModelCombo.Text;
        // TODO: Add tools configuration
        ((BlankViewModel)DataContext).AddAgent(agent);
        this.Close();
    }
    ...
    private string GetDefaultInstructions(PredefinedAgentType agentType)
    {
        return agentType.GetDefaultInstructions();
    ...
    {
        // Prefills the instructions box with the default instructions for the selected agent type. The user can then customize them as needed.
        PredefinedAgentType selectedType = (PredefinedAgentType)AgentTypeComboBox.SelectedItem;
        AgentInstructions.Text = GetDefaultInstructions(selectedType);
...
internal static class PredefinedAgentTypeExtensions
{
    internal static string GetDefaultInstructions(this PredefinedAgentType agentType)
    {
