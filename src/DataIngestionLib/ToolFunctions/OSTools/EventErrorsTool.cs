...
internal sealed class EventLogQuerySession : IEventLogQuerySession
{
	...
	{
		EventLogQuery eventLogQuery = new(logName, PathType.LogName, query)
		{
...
public sealed class EventErrorsTool
{
	...
	public EventErrorsTool()
		: this(new EventLogQuerySession())
	{
	}
	...
	public ToolResult<IReadOnlyList<EventErrorEntry>> ReadRecentCriticalAndErrorEvents([Description("Number of hours to look back for critical and error events. Range: 1 to 24.")] int lookbackHours = DEFAULT_LOOKBACK_HOURS)
	{
		...
		try
		{
			...
			var newestEvents = matchingEvents
				.OrderByDescending(entry => entry.TimeCreated ?? DateTime.MinValue)
				.ThenBy(entry => entry.LogName, StringComparer.OrdinalIgnoreCase)
				.Take(MAX_EVENTS_TO_RETURN)
				.ToList()
				.AsReadOnly();
	...
	internal static IReadOnlyList<string> GetCandidateLogs(IReadOnlyList<string> availableLogs)
	{
		return availableLogs
			.Where(logName => !string.IsNullOrWhiteSpace(logName))
			.OrderBy(logName => GetLogPriority(logName))
			.ThenBy(logName => logName, StringComparer.OrdinalIgnoreCase)
