...
public sealed class LogFileListingTool
{
    ...
    public ToolResult<IReadOnlyList<LogFileInfo>> GetLogFileList()
    {
        try
        {
            ...
            ToolResult<IReadOnlyList<string>> rootsResult = PathResolver.GetExistingDirectoryRoots(_allowedRoots);
            ...
            return ToolResult<IReadOnlyList<LogFileInfo>>.Ok(
                results.OrderByDescending(f => f.LastModifiedUtc).ToList()
            );
        }
        catch (Exception ex)
        {
