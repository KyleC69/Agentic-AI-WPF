...
- Bind `DataContext` in XAML for better separation of concerns and maintainability.
...
-- User identity is only needed for key in Chat History database storage and retrieval.

## External integrations
- There are to be no external integrations or dependencies in this project. All code should be self-contained and not rely on any external services or APIs. Application should operate in an air-gapped environment, private network only.
...
- Unit tests should focus on behavior and edge cases, not just code coverage. Use `InternalsVisibleTo` for testing internal methods when necessary, but prefer testing through public APIs.
...
- Do NOT alter the headers that appear at the top of the page in existing files; They are auto-generated by an extension and contain macros that would be lost if manually edited.
...
- Context injection (ChatHistory and Remote Knowledge RAG store) uses a custom "ChatRole" enum called "AIChatRole" to mark and track context injected messages. The 2 new Roles are AIContext for chat history injected as context, and RAGContext for injected context from the RAG system. This allows for better tracking and handling of context messages throughout the system, and avoids overloading the existing ChatMessage properties with additional metadata. **Any** injected messages must use one of these roles and immediately remove those same messages during the round-trip and not store duplicates or add extra noise in the context. 
